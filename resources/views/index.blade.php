...
@section('content')
    ...
    <section class="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8 p-8">
        <div class="text-center p-6 bg-white rounded-lg shadow-lg border border-gray-200 transform scale-100 hover:scale-105 transition duration-300 max-w-full">
            ...
            @if($latestBlogPost)
                <img src="{{ asset('images/blog/' . $latestBlogPost->image_path) }}" alt="{{ $latestBlogPost->title }}" class="w-full rounded-lg h-64 object-cover mb-4">
        ...
        <div class="text-center p-6 bg-white rounded-lg shadow-lg border border-gray-200 transform scale-100 hover:scale-105 transition duration-300 max-w-full">
            ...
            @if($latestNewsPost)
                <img src="{{ asset('images/news/' . $latestNewsPost->image_path) }}" alt="{{ $latestNewsPost->title }}" class="w-full rounded-lg h-64 object-cover mb-4">
                <p class="text-lg text-black mb-4">{{ $latestNewsPost->title }}</p>
                <a href="/news/{{ $latestNewsPost->slug }}" class="inline-block px-4 py-2 mt-6 text-white bg-green-700 rounded-full shadow-md hover:bg-green-500 hover:cursor-pointer transition duration-300">Read More</a>
            @else
            ...
            <section class="p-8">
                <div class="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-8">
                    ...
                    <div class="relative bg-gray-100 rounded-lg overflow-hidden shadow-md border-2 border-gray-300">
                        ...
                        <div class="absolute inset-x-0 bottom-0 h-1/2 bg-black bg-opacity-50 flex flex-col justify-center items-center p-4">
                            <p class="text-white text-lg font-semibold text-center mb-4"><?php echo $item['title']; ?></p>
                            <a href="<?php echo $item['href']; ?>" class="px-6 py-2 bg-green-600 text-white rounded-full hover:bg-green-500 transition duration-300">
                                    <?php echo $item['alt']; ?>
                            </a>
                        </div>
                    </div>
