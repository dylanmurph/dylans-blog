    ...
    <div class="w-4/5 m-auto pt-20">
        <form action="/reviews/{{ $review->slug }}" method="POST" enctype="multipart/form-data">
            ...
            <select name="equipment_type" class="w-full mt-8 p-3 border border-gray-300 rounded-lg">
                ...
                <option value="fairway" {{ $review->equipment_type == 'fairway' ? 'selected' : '' }}>Fairway Woods
                </option>
            </select>
            ...
            <div class="mt-8">
                ...
                <select name="rating" class="w-full p-3 border border-gray-300 rounded-lg">
                    @for ($i = 1; $i <= 5; $i++)
                        <option value="{{ $i }}" {{ $review->rating == $i ? 'selected' : '' }}>{{ $i }}
                            Star{{ $i > 1 ? 's' : '' }}</option>
                    @endfor
                </select>
            </div>
            ...
            <textarea name="pros"
                      class="py-10 bg-transparent block border-b-2 w-full h-40 text-xl outline-none mt-8">{{ $review->pros }}</textarea>

            <textarea name="cons"
                      class="py-10 bg-transparent block border-b-2 w-full h-40 text-xl outline-none mt-8">{{ $review->cons }}</textarea>
